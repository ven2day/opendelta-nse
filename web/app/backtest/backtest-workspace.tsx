      ...
      {strategies.loading || universes.loading ? <LoadingState label="Loading strategies and universes" /> : strategies.error ? <RequestErrorState error={strategies.error} retry={strategies.reload} /> : !strategy ? <EmptyState title="No strategies for this market" description={`No registered strategy supports ${marketLabel(market)}.`} /> : <form onSubmit={submit} noValidate>
        <div className="quant-panel-body">
          ...
          <div className="quant-form-grid quant-backtest-source-grid">
            <label><span>Symbol source</span><select value={effectiveSymbolSource} onChange={(event) => setSymbolSource(event.target.value)}><option value="universe">{activeUniverse ? `Active universe · ${activeUniverse.name} (${activeUniverse.symbols.length})` : "Active universe (none saved)"}</option>{(presets.data?.presets ?? []).map((preset) => <option key={preset.presetId} value={preset.presetId}>{preset.name} ({preset.symbols.length})</option>)}<option value="custom">Custom symbol list</option></select>{universes.error && <small>Saved universes unavailable: {universes.error.message}</small>}{presets.error && <small>Ready-made universes unavailable: {presets.error.message}</small>}</label>
            {effectiveSymbolSource === "custom" ? <label><span>Symbols</span><input value={customSymbols} placeholder="RELIANCE, TCS, INFY" onChange={(event) => setCustomSymbols(event.target.value)} /><small>{symbols.length} symbols selected</small></label> : <div className="quant-universe-summary"><div><span>Universe</span><strong>{selectedPreset?.name ?? activeUniverse?.name ?? "No active universe"}</strong><small>{selectedPreset ? `${symbols.length} symbols · official snapshot ${selectedPreset.asOf}` : symbols.length ? `${symbols.length} symbols ready` : "Choose a ready-made universe or save one in Screener"}</small></div><a href={market === "CRYPTO" ? "/screener?market=CRYPTO" : "/screener"}>Manage in Screener</a></div>}
          </div>
