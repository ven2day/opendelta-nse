...
export function BacktestWorkspace({ market, initialRunId }: { market: PlatformMarket; initialRunId?: string }) {
  ...
  const strategyOptions = useMemo<BacktestStrategyOption[]>(() => [
    ...
    ...(strategySources.data?.sources ?? []).map((source) => ({
      ...
      strategy: {
        strategyId: source.strategyId, name: source.name, version: source.strategyVersion,
  ...
  const approvalSource = run?.strategySourceId ? "OPENDELTA" : deployments.data?.deployments.find((item) => item.strategyId === run?.strategyId)?.signalSource ?? "OPENDELTA";
    ...
    <Panel icon={<FlaskConical size={17} />} title="Run backtest">
      {strategies.loading || universes.loading || strategySources.loading ? <LoadingState label="Loading strategies and universes" /> : strategies.error ? <RequestErrorState error={strategies.error} retry={strategies.reload} /> : !strategy ? <EmptyState title="No strategies for this market" description={`No registered strategy supports ${marketLabel(market)}.`} /> : <form onSubmit={submit} noValidate>
        <div className="quant-panel-body">
          ...
          {strategySources.error && <p className="quant-inline-note">Strategy sources unavailable: {strategySources.error.message}</p>}
          {selectedStrategyOption?.strategySourceId && <p className="quant-inline-note">Strategy code runs in the isolated, networkless runner. A completed run can be approved for OpenDelta Signals, then Paper.</p>}
