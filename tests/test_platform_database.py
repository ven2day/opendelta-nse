...
class PlatformDatabaseTests(unittest.TestCase):
    ...
    def _run(self, **overrides):
        values = dict(market="NSE", strategy_id="ema_vwap_strong_buy", strategy_version="1.0.0", configuration_snapshot={"target_pct": 1.0}, execution_settings={"batchSize": 500}, timeframe="5m", symbols=["RELIANCE", "TCS"], start_date=date(2026, 8, 1), end_date=date(2026, 8, 31))
    ...
    def test_migrations_are_versioned_and_idempotent(self) -> None:
        self.assertEqual(
            ...
            ["001_platform", "001_timescale_market_data", "002_timescale_candle_reader", "003_backtest_trade_last_price", "004_paper_pending_entries", "005_dhan_fifo_cost_basis", "006_live_signal_strategy_identity"],
        )
    ...
    def test_run_lifecycle_and_progress(self) -> None:
        ...
        self.runs.update_progress(record["runId"], symbols_completed=1, current_symbol="TCS", failed_symbols=[{"symbol": "RELIANCE", "message": "no data"}])
        progressed = self.runs.get(record["runId"])
        self.assertEqual((progressed["status"], progressed["symbolsCompleted"], progressed["currentSymbol"]), ("RUNNING", 1, "TCS"))
