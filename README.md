...
- **🔄 Backtest** — background jobs, one symbol at a time with bounded memory, next-candle-open entries, independent target tranches, Dhan-compatible FIFO cost allocation for NSE sells, fees and slippage on both sides, MAE/MFE, drawdown, cancellation, failed-symbol isolation, trades written to PostgreSQL in batches. NSE daily signals replay against completed 5-minute candles for paper-aligned entry and target-hit timestamps.
...
- **💼 Paper Trading** — internal `PaperBroker`, strategy-specific immutable execution policies, fixed-quantity or fixed-capital sizing with Strong Buy lot multipliers, one order per signal, candle-driven exits, Dhan-compatible FIFO realized/unrealized P&L for NSE, portfolio/pending-order recovery after a restart, and account-wide position, daily-trade, daily-loss and exposure guards.
...
The suites prove, among other things: backtest and signal evaluation agree bar
...
independently; and no broker or exchange order-submission implementation is present.
...
OpenDelta is the default UI/API platform. Signal and paper activity still requires an
operator-reviewed durable deployment. Paper trading is the only execution mode. See
