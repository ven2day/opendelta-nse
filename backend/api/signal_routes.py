...
def create_signal_router(
    *,
    ...
    worker_statuses: Callable[[str], list[dict[str, Any]]],
) -> APIRouter:
    ...
    def list_signals(
        ...
        rows = _guard(signals).list(
            market_key,
            status=status_key,
            symbol=symbol.strip().upper() if symbol else None,
            strategy_id=strategy.strip() if strategy else None,
    ...
    def signal_health(market: str | None = Query(default=None)) -> dict[str, Any]:
        ...
        live = {key: worker_statuses(key) for key in (MARKETS if market_key is None else (market_key,))}
        return {"engines": stored, "workers": live, "paperOnly": True, "liveOrdersEnabled": False}
