...
- **📉 RSI Dip Ladder** — one NSE RSI-recovery entry starts a finite four-lot ladder; later lots trigger from completed-candle 5% dips without requiring another RSI signal, using price-band quantities (`5/10/25/50` from ₹1,000; `10/20/50/100` below ₹1,000), a ₹250,000 open-capital guard, and an independent configurable target for every lot.
- **🧾 Dhan FIFO accounting** — a target tranche decides the sell quantity, while NSE inventory is matched oldest-first for realized P&L. The remaining cost basis is recalculated after every partial sale; the original tranche entry remains visible beside the FIFO cost.
