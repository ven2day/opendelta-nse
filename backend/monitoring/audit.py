...
_ACTIONS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("STRATEGY_VALIDATED", re.compile(r"^/v2/strategy-sources/[^/]+/validate$")),
    ("STRATEGY_DRAFT_SAVED", re.compile(r"^/v2/strategy-sources$")),
    ("INDICATOR_VALIDATED", re.compile(r"^/v2/indicator-sources/[^/]+/validate$")),
    ("INDICATOR_DRAFT_SAVED", re.compile(r"^/v2/indicator-sources$")),
    ("BACKTEST_CANCELLED", re.compile(r"^/v2/backtests/[^/]+/cancel$")),
    ("BACKTEST_CREATED", re.compile(r"^/v2/backtests$")),
    ("EXPERIMENT_CANCELLED", re.compile(r"^/v2/research/experiments/[^/]+/cancel$")),
    ("EXPERIMENT_CREATED", re.compile(r"^/v2/research/experiments(?:/from-preview)?$")),
    ("WALK_FORWARD_CANCELLED", re.compile(r"^/v2/research/walk-forward/[^/]+/cancel$")),
    ("WALK_FORWARD_CREATED", re.compile(r"^/v2/research/walk-forward(?:/from-preview)?$")),
    ("SIGNALS_APPROVAL", re.compile(r"/approve-signals$")),
    ("PAPER_APPROVAL", re.compile(r"/approve-paper$")),
    ("CREDENTIAL_CHANGED", re.compile(r"^/v2/exchange-connections(?:/[^/]+)?$")),
    ("CREDENTIAL_TESTED", re.compile(r"^/v2/exchange-connections/[^/]+/test$")),
