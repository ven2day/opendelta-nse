...
| `backend/data/store.py` | Explicit legacy/fallback candle-read modes during Timescale rollout | Strict Timescale is the default, but rollback still consumes this store | fallback tests; no destructive migration | KEEP as a rollback reader until an operational retention decision. |
...
| `PLATFORM_CANDLE_READ_MODE=legacy` and `timescale-fallback` options | Operator rollback and unconfigured local development | strict `timescale` is the default | Timescale/fallback tests; production data readiness required | Keep options until an owner-approved retention window ends. |
